...
func MakeBuildController(kubeClient client.Interface, buildClient BuildInterface, dockerBuilderImage,
	dockerRegistry, stiBuilderImage string, timeout int) *BuildController {
	glog.Infof("Creating build controller with dockerBuilderImage=%s, dockerRegistry=%s, "+
...
func (bc *BuildController) synchronize() {
	builds, err := bc.buildClient.ListBuilds()
	if err != nil {
	...
	for _, build := range builds.Items {
		...
		if nextStatus != build.Status {
			...
			if _, err := bc.buildClient.UpdateBuild(build); err != nil {
